# This code is revised from the original version of Qiskit for applying dynamical decoupling sequences by heumtum(2022 Hackathon)
...
from CT_DD import MyDynamicalDecoupling


LOG = logging.getLogger(__name__)


class StandardRB_dd(BaseExperiment, RestlessMixin):
    ...
    def __init__(
        ...
        dd_sequence_target: Sequence[Gate]=None,
        dd_sequence_control: Sequence[Gate]=None,
    ):
        ...
        #dd_sequences
        if dd_sequence_target is None:
            self.dd_sequence_target=[XGate(),XGate(),XGate(),XGate()]
        else:
            self.dd_sequence_target=dd_sequence_target
        if dd_sequence_control is None:
            self.dd_sequence_control=self.dd_sequence_target
        else:
            self.dd_sequence_control=dd_sequence_control
    ...
    def _transpiled_circuits(self) -> List[QuantumCircuit]:
        ...
        #copied from _transpiled_circuits of base experiment
        transpile_opts = copy.copy(self.transpile_options.__dict__)
        transpile_opts["initial_layout"] = list(self.physical_qubits)
        
        #apply dd using passmanager
        basis_gates=self.backend.configuration().basis_gates
        coupling_map=self.backend.configuration().coupling_map
        durations=InstructionDurations.from_backend(self.backend)
        pm=PassManager([ALAPSchedule(durations), MyDynamicalDecoupling(durations, dd_sequence_target=self.dd_sequence_target,dd_sequence_control=self.dd_sequence_control,backend=self._backend)])
        transpiled=[]        
        for i in self.circuits():            
            transpiled_circuit=transpile(i,self.backend,**transpile_opts)
            dd=pm.run(transpiled_circuit)
            transpiled.append(dd)  
